...
BEGIN
	...
	DECLARE @inventory_id INTEGER, @inventory_target_id INTEGER, @asset_domain_id SMALLINT, @asset_id INTEGER, @budget_qty INTEGER,
		...
		@serial_name_ow BIT, @lan INT, @lan_ow BIT, @network_type VARCHAR(20), @network_type_ow BIT;
	...
	SELECT @inventory_id = [inventory_id], @inventory_target_id = [inventory_target_id], @asset_domain_id = [asset_domain_id], @asset_id = [asset_id], @budget_qty = [budget_qty],
		...
		@manufacturer_description = manufacturer_description, @manufacturer_description_ow = manufacturer_description_ow, @serial_number = serial_number,
		@serial_number_ow = serial_number_ow, @serial_name = serial_name, @serial_name_ow = serial_name_ow, @lan = lan, @lan_ow = lan_ow, @network_type = network_type, @network_type_ow = network_type_ow
