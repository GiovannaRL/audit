...
CREATE PROCEDURE [dbo].[procurement_chart2](
    @project_id integer,
    @phase_id integer,
...
BEGIN
	...
	declare
		...
		if @department_id is not null or @phase_id is not null or @cost_center_id is not null begin
			...
			from ancillary_v a
				inner join inventory_w_relo_v b on a.project_id = b.project_id and b.asset_domain_id in (@domain_id1, @domain_id2)
			where a.project_id = @project_id;
		end;
		...
		while (@period_end_date <= @v_end_date) begin
			select @po_amount = coalesce(sum(base.po_subtotal + base.ipo_subtotal), 0)
			from
				(
				...
				insert into @return_table values(@period_end_date, @projected_budget, @po_amount);
