...
{
    class ComprehensiveInteriorDesignReport : GenericReportRepository
    {
        public void Build(project_report item)
        {
            ...
            List<ComprehensiveInteriorDesignItem> data = GetData(item, 20);
        ...
        public string BuildPDF(project_report item, string filename, List<ComprehensiveInteriorDesignItem> data, Decimal totalPercentage)
        {
            ...
            if (data.Count() == 0)
            {
                stringWriter.Write("<p>There is no inventories for this project.</p>");
            }
            ...
            foreach (var sheet in data)
            {
                ...
                if (!string.IsNullOrEmpty(sheet.blob_file_name) || !string.IsNullOrEmpty(sheet.photo))
                {
                    //DOWNLOAD PHOTO
                    var asset_photo = sheet.blob_file_name ?? sheet.photo;
                    var asset_photo_domain_id = String.IsNullOrEmpty(sheet.blob_file_name) ? sheet.asset_domain_id : sheet.domain_id;

                    var blob_photo = _fileRepository.GetBlob(BlobContainersName.Photo(asset_photo_domain_id), asset_photo);
                    ...
                    var full_photo_dir = Path.Combine(photo_dir, guid + asset_photo);
                ...
                if (sheet.options.Count() > 0)
                {
                    ...
                    foreach (var option in sheet.options)
                    {
                        ...
                        {
                            settings = JsonConvert.DeserializeObject<OptionSettings>(option.settings.Replace("\"MAT - Finish\":", "\"mat_finish\":").Replace("\"Dimensions\":", "\"dimensions\":").Replace("\"Type Name\":", "\"type_name\":"));
                        }
                        ...
                        stringWriter.Write("<td style=\"vertical-align:center\"><strong>FRAME TYPE:</strong> Frame<br/><br/>");
                        stringWriter.Write("<strong>DIMENSIONS:</strong> " + settings.dimensions + "<br/><br/>");
                        stringWriter.Write("<strong>FRAME COLOR:</strong> " + option.display_code + "<br/><br/>");
                        stringWriter.Write("<strong>MAT FINISH:</strong> " + settings.mat_finish + "<br/><br/>");
                        stringWriter.Write("<strong>TYPE NAME:</strong> " + settings.type_name + "<br/><br/>");
                        stringWriter.Write("<strong>NOTES:</strong> " + option.description + "</td>");
                        stringWriter.Write("</tr>");
                ...
                /* ARTWORK */
                if (sheet.artworks.Count() > 0)
                {
                    var totalPerLine = 2;
                    var perLine = 0;
                    stringWriter.Write("<tr><td style=\"align:center;width:100%\"><table align=\"center\" cellpadding=\"5\" class=\"noborder\"><tr>");
                    foreach (var option in sheet.artworks)
                    {
                        perLine++;
                        var option_img = "<img src='' BorderStyle='None' AlternateText='No Picture' Height='200' Width='200' />";
                        if (!string.IsNullOrEmpty(option.blob_file_name))
                        {
                            //DOWNLOAD PHOTO
                            var blob_photo = _fileRepository.GetBlob(BlobContainersName.Photo(option.project_domain_id), option.blob_file_name);
                        ...
                        stringWriter.Write("<table width=\"100%\" align=\"center\" class=\"noborder\">");
                        stringWriter.Write("<tr>");
                        stringWriter.Write("<td><strong>" + option.label + ":</strong></td>");
                        stringWriter.Write("</tr>");
                        stringWriter.Write("<tr>");
                        stringWriter.Write("<td>" + option_img + "</td>");
                        stringWriter.Write("</tr>");
                ...
                stringWriter.Write("<td width=\"110px\"><img border='0' height=\"40\" src='" + Path.Combine(Domain.GetRoot(), "logo_aw.png") + "'></td>");
                stringWriter.Write("</tr>");
                stringWriter.Write("</table>");
                stringWriter.Write("</td>");
                stringWriter.Write("</tr>");
        ...
        private List<ComprehensiveInteriorDesignItem> GetData(project_report report, decimal total_percentage)
        {
            ...
            select.Append("WHERE da.inventory_id = pri.inventory_id and pdoc.type_id <> 8 order by da.id ");
            ...
            select.Append("inner join assets_options ao on ao.asset_option_id = io.option_id and ao.domain_id = io.domain_id and ao.data_type = 'FR' ");
            ...
            select.Append(" WHERE pri.jsn_code is not null and pri.net_new > 0 ");
            select.Append(where.Replace("WHERE", "AND"));

            List<ComprehensiveInteriorDesignItem> items = this._db.Database.SqlQuery<ComprehensiveInteriorDesignItem>(select.ToString()).ToList();
            ...
            if (qty > 0)
            {
                ...
                for (var i = 0; i < qty; i++)
                {
                    ...
                    select.Append("WHERE ao.data_type = 'FR' AND io.inventory_id = " + items[i].inventory_id);
                    ...
                    /* GET ARTWORK DOCUMENTS FOR THIS INVENTORY */
                    select.Clear();
                    select.Append("SELECT pdoc.blob_file_name, pdoc.label, pdoc.project_domain_id from documents_associations da ");
                    select.Append("INNER JOIN project_documents pdoc on pdoc.id = da.document_id and pdoc.project_domain_id = da.project_domain_id ");
                    select.Append("WHERE da.inventory_id = " + items[i].inventory_id);
                    select.Append(" AND da.project_domain_id = " + items[i].domain_id);
                    select.Append(" AND pdoc.type_id = " + DocumentTypeIdEnum.Artwork);

                    List<ArtworkItem> artworks = this._db.Database.SqlQuery<ArtworkItem>(select.ToString()).ToList();
                    items[i].artworks = artworks;
